...
void SystemTray::setupMenu() {
    ...
    // 语言设置子菜单
    m_languageMenu = m_contextMenu->addMenu(tr("语言"));
    ...
    // 高级子菜单
    m_advancedMenu = m_contextMenu->addMenu(tr("高级"));

    // 窗口置顶
    m_alwaysOnTopAction = m_advancedMenu->addAction(tr("窗口置顶"));
    ...
    // 开机启动
    m_autoStartAction = m_advancedMenu->addAction(tr("开机启动"));
    ...
    // 自动移除已下载项子菜单
    m_autoDeleteMenu = m_advancedMenu->addMenu(tr("自动移除已下载项"));
    ...
    // 调试日志
    m_debugLogAction = m_advancedMenu->addAction(tr("调试日志"));
